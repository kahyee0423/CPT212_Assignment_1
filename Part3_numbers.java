...
public class Part3_numbers {
    ...
    public static void radixSort(int[] numbers) {
        ...
        for (int i = 0; i < n; i++) {
            ...
            counter += 4; //1 assign, 1 method call, 1 obj reference, 1 lookup
        ...
        for (int digitPlace = 0; digitPlace < maxDigits; digitPlace++) {
            ...
            for (int i = 0; i < n; i++) {
                ...
                counter +=5; //1 assign, 1 lookup, 1 method call, 2 op
                ...
                counter += 3; // 1 assign, 1 lookup, 1 op
            }
            counter += 2;
            ...
            for (int i = 0; i < 10; i++) {
                ...
                for (int j = 0; j < count[i]; j++) {
                    counter += 4; //1 assign, 1 compare, 1 lookup, 1 op

                    numbers[index++] = arrayBoxes[i][j]; // Reorder numbers back into the original array
                    counter += 6; // 2 assign, 1 op, 3 lookup
                }
            }
